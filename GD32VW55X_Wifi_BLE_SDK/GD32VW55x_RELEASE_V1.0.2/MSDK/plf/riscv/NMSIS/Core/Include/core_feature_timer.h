...
typedef struct {
    ...
    union {
        __IOM uint64_t MTIMERCMP_64;        /*!< Offset: 0x008 (R/W)  System Timer compare Value 64bits Register */
        struct {
            __IOM uint32_t MTIMERCMP_lo;    /*!< Offset: 0x008 (R/W)  System Timer compare Value low 32bits Register */
            __IOM uint32_t MTIMERCMP_hi;    /*!< Offset: 0x00C (R/W)  System Timer compare Value high 32bits Register */
        } MTIMERCMP_32;
    } MTIMERCMP;
...
__STATIC_FORCEINLINE void SysTimer_SetCompareValue(uint64_t value)
{
    ...
    /* If the low 32bits of reload_ticks are smaller than cur_ticks, the high 32bits are written first in MTIMERCMP,
     * otherwise the low 32bits are written first to avoid unexpected interrupt. */
    if (__USUALLY((uint32_t)value > (uint32_t)cur_ticks)) {
        SysTimer->MTIMERCMP.MTIMERCMP_64 = value;
    } else {
        SysTimer->MTIMERCMP.MTIMERCMP_32.MTIMERCMP_hi = (uint32_t)(value >> 32);
        SysTimer->MTIMERCMP.MTIMERCMP_32.MTIMERCMP_lo = (uint32_t)(value & 0xFFFFFFFF);
    }
}
...
{
    return SysTimer->MTIMERCMP.MTIMERCMP_64;
}
...
__STATIC_FORCEINLINE uint32_t SysTick_Reload(uint64_t ticks)
{
    ...
    if (__USUALLY(reload_ticks > cur_ticks)) {
        /* If the low 32bits of reload_ticks are smaller than cur_ticks, the high 32bits are written first in MTIMERCMP,
         * otherwise the low 32bits are written first to avoid unexpected interrupt. */
        if (__USUALLY((uint32_t)reload_ticks > (uint32_t)cur_ticks)) {
            SysTimer->MTIMERCMP.MTIMERCMP_64 = reload_ticks;
        } else {
            SysTimer->MTIMERCMP.MTIMERCMP_32.MTIMERCMP_hi = (uint32_t)(reload_ticks >> 32);
            SysTimer->MTIMERCMP.MTIMERCMP_32.MTIMERCMP_lo = (uint32_t)(reload_ticks & 0xFFFFFFFF);
        }
    } else {
        ...
        SysTimer->MTIMER = 0;
        SysTimer->MTIMERCMP.MTIMERCMP_64 = ticks;
    }
